...
function ScheduleList({ entries, onToggleDone, onDelete, emptyText = 'No schedule entries yet.' }) {
  if (!entries.length) {
    return <p className="px-1 py-6 text-sm text-zinc-400">{emptyText}</p>;
  }
  ...
  return (
    <div className="space-y-5">
      {groupByDate(entries).map((group) => (
        <div key={group.date}>
          <p className="mb-2 text-sm font-semibold text-zinc-400">{formatDate(group.date)}</p>
          <ul className="space-y-2">
            {group.items.map((entry) => (
              <li
                key={entry.id}
                className="flex items-start justify-between gap-3 rounded-xl border border-white/10 bg-white/[0.05] p-3 transition hover:border-brand-500/40"
              >
                  ...
                  {onToggleDone ? (
                    ...
                    <span
                      className={`mt-1 inline-block h-3 w-3 flex-none rounded-full ${
                        entry.done ? 'bg-emerald-500' : 'bg-white/20'
                      }`}
